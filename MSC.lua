...
table.sort(players, function(a, b)
    local classA = CLASS_INDEX[a.class] or 99
    local classB = CLASS_INDEX[b.class] or 99
...
for i = 1, 25 do
    ...
    if row then
        if player then
            local r, g, b = GetClassColor(player.unitID)
            row.nameText:SetTextColor(r, g, b)
            row.nameText:SetText(player.name)
            row.specText:SetText((notes and notes[player.name]) or "--")
            row.unitClass = player.class
            row:Show()
        else
            row:Hide()
        end
...
verBtn:SetScript("OnClick", function()
    ...
    else Print("You must be in a raid group to check versions.") end
end)
