...
MSCDB = MSCDB or {}
MSCOptions = MSCOptions or { showMinimap = true, minimapPos = 45 }

local MSC_VERSION = "0.1.2"
local MSG_PREFIX = "MSC_VER"
local notes = MSCDB
local selectedPlayer = nil

-- Logic variables for Version Check (Defined here for global file access)
...
UIDropDownMenu_Initialize(MSCDropdown, function(self, level)
    ...
    for i=1, 25 do
        local r = _G["MSCRow"..i]
        if r.nameText:GetText() == selectedPlayer then
            pClass = r.unitClass
            break
        end
    end
...
report:SetScript("OnClick", function()
    ...
    for name, spec in pairs(notes) do 
        if IsInGroup(name) then table.insert(members, name .. " (" .. spec .. ")") end
    end
...
-- --------------------------------------------------------
-- Options Window (Fixed Layout)
-- --------------------------------------------------------
local opt = CreateFrame("Frame", "MSCOptionsFrame", UIParent)
opt:SetSize(300, 280) -- Increased height for better spacing
opt:SetPoint("CENTER")
opt:SetBackdrop({
...
-- 2. Checkbox (Moved further down to avoid overlap)
local hideCheck = CreateFrame("CheckButton", "MSCHideCheck", opt, "InterfaceOptionsCheckButtonTemplate")
hideCheck:SetPoint("TOPLEFT", 15, -150) -- Lowered from -110 to -150
...
hideCheck:SetScript("OnClick", function(self)
    ...
    if MSCOptions.showMinimap then MSCMinimapButton:Show() else MSCMinimapButton:Hide() end
end)

-- 3. The Test Button (Bottom)
...
mini:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine("|cffffffffVersion: |r" .. MSC_VERSION)
    GameTooltip:AddLine("Left-Click: |cffffffffToggle Menu|r")
    GameTooltip:AddLine("Middle-Click: |cffffffffOptions|r")
    GameTooltip:AddLine("Right-Click: |cffffffffClose Menu|r")
    GameTooltip:Show()
end)
...
init:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_LOGIN" then
        ...
        -- Sync icon visibility with options
        UpdateMinimapPosition()
        if MSCOptions.showMinimap == false then mini:Hide() else mini:Show() end
        hideCheck:SetChecked(MSCOptions.showMinimap)
        
        Print("MSC Loaded (v" .. MSC_VERSION .. ")")
    elseif event == "PARTY_MEMBERS_CHANGED" or event == "RAID_ROSTER_UPDATE" then
        if GetNumRaidMembers() == 0 and GetNumPartyMembers() == 0 then
            if next(MSCDB) ~= nil then
    ...
    elseif event == "CHAT_MSG_ADDON" then
        ...
        if prefix == MSG_PREFIX then
            ...
            elseif message:find("REPLY:") and verCheckActive then
                verResults[sender] = message:gsub("REPLY:", "")
            end
        end
...
-- --------------------------------------------------------
-- Updated Slash Commands (With Sync Logic)
-- --------------------------------------------------------
SlashCmdList["MSC"] = function(msg)
    msg = msg:lower() -- Makes it case-insensitive (e.g., /msc SHOW works)
    
    if msg == "reset" then
        MSCOptions.x, MSCOptions.y = nil, nil
        f:ClearAllPoints()
        f:SetPoint("CENTER")
        Print("Window position reset.")
    elseif msg == "show" then
        MSCOptions.showMinimap = true
        MSCMinimapButton:Show()
        MSCHideCheck:SetChecked(true)
        Print("Minimap icon shown.")
    elseif msg == "hide" then
        MSCOptions.showMinimap = false
        MSCMinimapButton:Hide()
        MSCHideCheck:SetChecked(false)
